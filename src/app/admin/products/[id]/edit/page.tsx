...
export default function EditProduct() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (!saveRes.ok) {
      setError("فشل تحديث المنتج: " + (saveData.error || "خطأ غير معروف"));
      setSaving(false);
      return;
    }
    ...
    <div className="p-6 max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">عدّل بيانات المنتج وصوره</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-5">
        ...
        <div className="bg-[#0f1320] border border-purple-500/20 rounded-2xl p-6">
          ...
          <label className="flex flex-col items-center justify-center border-2 border-dashed border-purple-500/30 rounded-xl p-6 cursor-pointer hover:border-purple-500/60 transition-colors">
            ...
            <span className="text-sm text-gray-400">اضغط لإضافة صور</span>
            ...
            <div className="grid grid-cols-4 gap-3 mt-4">
              {existingImages.map((src, i) => (
                <div key={`ex-${i}`} className="relative group">
                  ...
                  <button type="button" onClick={() => removeExisting(i)} className="absolute top-1 right-1 w-6 h-6 bg-red-500 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <X size={12} />
                  </button>
                </div>
              ))}
              {newPreviews.map((src, i) => (
                <div key={`new-${i}`} className="relative group">
                  ...
                  <button type="button" onClick={() => removeNew(i)} className="absolute top-1 right-1 w-6 h-6 bg-red-500 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <X size={12} />
                  </button>
                </div>
              ))}
