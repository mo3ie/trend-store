...
export default function EmployeesPage() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-screen bg-[var(--bg)]">
      <p className="text-[var(--muted-2)]">⏳ جاري التحميل...</p>
    </div>
  );

  return (
    <div className="p-8 min-h-screen bg-[var(--bg)] text-[var(--text)]" dir="rtl">
      <div className="flex items-center justify-between mb-8 pb-6 border-b border-purple-500/20">
        <div>
          ...
          <p className="text-[var(--muted-2)] text-sm mt-1">{employees.length} موظف</p>
        </div>
        <button
          onClick={() => setShowModal(true)}
          className="px-5 py-2.5 bg-purple-600 hover:bg-purple-500 text-[var(--text)] font-bold rounded-xl transition-colors text-sm"
        >
          + إضافة موظف
        </button>
      </div>
      ...
      {employees.length === 0 ? (
        ...
        <div className="grid gap-5">
          {employees.map(emp => (
            <div key={emp.id} className="bg-[var(--surface)] border border-purple-500/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-5">
                <div>
                  <p className="font-bold text-lg">{emp.full_name}</p>
                  <p className="text-[var(--muted-2)] text-sm">موظف</p>
                </div>
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2.5 p-3 rounded-xl border text-sm text-right transition-all ${
                        val
                          ? "border-purple-500/40 bg-purple-600/15 text-[var(--text)]"
                          : "border-[var(--border)] bg-white/5 text-[var(--muted-2)] hover:text-[var(--muted)]"
                      }`}
                    >
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-[var(--surface)] border border-purple-500/30 rounded-2xl p-8 w-[380px]">
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl bg-[var(--input)] border border-purple-500/20 text-[var(--text)] placeholder:text-[var(--muted-2)] focus:outline-none focus:border-purple-500/60 text-sm mb-3"
            />
            <input
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl bg-[var(--input)] border border-purple-500/20 text-[var(--text)] placeholder:text-[var(--muted-2)] focus:outline-none focus:border-purple-500/60 text-sm mb-5"
            />
            <div className="flex gap-3">
              ...
              <button
                ...
                className="px-5 py-3 bg-white/5 border border-[var(--border)] text-[var(--muted)] rounded-xl hover:bg-white/10 transition-colors"
              >
                إلغاء
              </button>
            </div>
