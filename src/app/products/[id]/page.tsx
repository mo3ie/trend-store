    ...
    <div className="flex flex-col bg-[var(--bg)] text-[var(--text)] min-h-screen">
      ...
      <header className="flex items-center gap-4 px-6 py-3.5 border-b border-purple-500/20 sticky top-0 bg-[var(--glass)] backdrop-blur-md z-40">
        ...
        <div className="flex-1">
          <input placeholder="ابحث عن منتج..." className="w-full px-4 py-2.5 rounded-xl bg-[var(--input)] border border-purple-500/20 text-sm focus:outline-none focus:border-purple-500/50 transition-colors placeholder:text-[var(--muted-2)]" />
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              ...
              <div className="space-y-3">
                <div className="relative bg-[var(--surface)] border border-purple-500/20 rounded-2xl overflow-hidden aspect-square">
                  ...
                  {images.length > 1 && (
                    <>
                      <button onClick={() => setActiveImage((p) => (p - 1 + images.length) % images.length)} className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/60 hover:bg-black/80 rounded-full flex items-center justify-center text-[var(--text)] transition-colors">
                        <ChevronLeft size={16} />
                      </button>
                      <button onClick={() => setActiveImage((p) => (p + 1) % images.length)} className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/60 hover:bg-black/80 rounded-full flex items-center justify-center text-[var(--text)] transition-colors">
              ...
              <div className="space-y-5">
                ...
                <div className="flex items-end gap-2">
                  ...
                  <span className="text-[var(--muted)] text-lg mb-1">دينار</span>
                </div>
                ...
                {product.description && (
                  <div className="bg-[var(--surface)] border border-purple-500/15 rounded-2xl p-4">
                    <p className="text-[var(--muted)] text-xs font-medium mb-2">وصف المنتج</p>
                  ...
                  <div className="space-y-4">
                    {product.variants.map((variant) => (
                      <div key={variant.name}>
                        <label className="text-[var(--muted)] text-xs font-medium mb-2 block flex items-center gap-1.5">
                          {variant.name}
                            ...
                            return (
                              <button
                                ...
                                className={`px-4 py-2 rounded-xl text-sm font-medium border-2 transition-all ${
                                  isSelected
                                    ? "border-purple-500 bg-purple-500/20 text-[var(--text)] shadow-[0_0_12px_rgba(168,85,247,0.4)]"
                                    : "border-[var(--border)] bg-white/5 text-[var(--muted)] hover:border-purple-500/40 hover:text-[var(--text)]"
                                }`}
                              >
                                {option}
                              </button>
                ...
                <div className="space-y-3 pt-2">
                  <button
                    ...
                    disabled={!inStock}
                    className={`w-full py-3.5 rounded-2xl font-bold text-[var(--text)] flex items-center justify-center gap-2 transition-all ${
                      inStock
                        ? added
                ...
                <div className="border-t border-purple-500/15 pt-4 space-y-2 text-xs text-[var(--muted-2)]">
                  <div className="flex justify-between"><span>التصنيف</span><span className="text-[var(--muted)]">{product.category}</span></div>
                  ...
                  {product.created_at && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-[var(--muted)]">{new Date(product.created_at).toLocaleDateString("ar-EG", { year: "numeric", month: "long", day: "numeric" })}</span>
                    </div>
                  )}
                </div>
              </div>
