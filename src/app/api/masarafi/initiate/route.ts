...
import { signin, openSession } from "@/lib/masarafi";
...
// POST /api/masarafi/initiate
// Body: { order_id, identityCard, amount }
// Signs in to Masarafi, opens a sale session (bank sends OTP to customer),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const masarafiSession = JSON.stringify({ token: sessionToken, transactionId, validTo });
    const { error: updErr } = await supabaseAdmin
      .from("store_orders")
      .update({ dpay_session_id: masarafiSession })
      .eq("id", order_id);
    ...
    console.error(`[masarafi/initiate] order ${order_id}:`, e.message);
