    ...
    <div>
      ...
      {addresses.length === 0 ? (
        <div className="text-center py-10 bg-black/20 rounded-2xl border border-purple-500/10">
          ...
          <p className="text-gray-500 text-sm">لا توجد عناوين محفوظة بعد</p>
        </div>
      ) : (
        <div className="space-y-3">
          {addresses.map(addr => (
            <div key={addr.id} className={`bg-[#0f1320] rounded-2xl p-4 border transition-all ${addr.is_default ? "border-purple-500/60" : "border-purple-500/20"}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-gray-400 text-sm">{addr.address_text}</p>
                  {addr.lat && <p className="text-gray-600 text-xs mt-1">📍 {addr.lat.toFixed(4)}, {addr.lng?.toFixed(4)}</p>}
                </div>
                <div className="flex gap-2 shrink-0">
                  {!addr.is_default && (
                    <button onClick={() => handleSetDefault(addr)} title="تعيين افتراضي" className="w-8 h-8 rounded-lg bg-white/5 hover:bg-yellow-500/10 border border-white/10 flex items-center justify-center text-gray-400 hover:text-yellow-400 transition-colors">
                      <Star size={13} />
                    </button>
                  )}
                  <button onClick={() => openEdit(addr)} className="w-8 h-8 rounded-lg bg-white/5 hover:bg-purple-500/10 border border-white/10 flex items-center justify-center text-gray-400 hover:text-purple-400 transition-colors">
                    <Edit3 size={13} />
                  </button>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setShowModal(false)}>
          <div className="bg-[#0f1320] border border-purple-500/20 rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <h3 className="font-bold text-lg mb-5">{editing ? "تعديل العنوان" : "إضافة عنوان جديد"}</h3>

            <div className="space-y-4">
              {/* Label Chips */}
              <div>
                <label className="text-gray-500 text-xs mb-2 block">تسمية العنوان</label>
                <div className="flex gap-2">
                  {["المنزل", "العمل", "آخر"].map(lbl => (
                    <button key={lbl} onClick={() => setForm(f => ({ ...f, label: lbl }))}
                      className={`flex-1 py-2 rounded-xl text-sm font-medium transition-all border ${form.label === lbl ? "border-purple-500 bg-purple-500/20 text-purple-300" : "border-white/10 bg-white/5 text-gray-400"}`}>
                      {lbl}
                    </button>
                  ))}
                </div>
              ...
              {/* Address Text */}
              <div>
                <label className="text-gray-500 text-xs mb-2 block">العنوان كنص</label>
              ...
              {/* Map */}
              <div>
                <label className="text-gray-500 text-xs mb-2 block">الموقع على الخريطة (اختياري)</label>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-gray-400 text-sm">تعيين كعنوان افتراضي</span>
              </label>
            </div>
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={() => setShowModal(false)}
                className="px-5 py-3 rounded-xl bg-white/5 border border-white/10 text-gray-400 hover:text-white transition-colors text-sm">
                إلغاء
              </button>
            </div>
          </div>
