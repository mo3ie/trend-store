...
## 6. Cron jobs — ⚠️ NOT in `vercel.json`

| Path | Schedule | Purpose |
|---|---|---|
| `/api/bot/cron/drain` | every 1 min | deliver throttle-deferred Facebook comments |
| `/api/tiktok/cron/poll` | every 2 min | poll TikTok for new comments |

**The Vercel Hobby plan allows only ONE cron run per day**, and defining a
faster schedule makes the deploy *hard-fail*. So there is deliberately no `crons`
block in `vercel.json` (the file was removed entirely).

Drive the two routes from an **external scheduler** instead — e.g. the free
[cron-job.org](https://cron-job.org):

```
GET https://trendstore-ly.com/api/bot/cron/drain      every 1 minute
GET https://trendstore-ly.com/api/tiktok/cron/poll    every 2 minutes
Header:  Authorization: Bearer <CRON_SECRET>
```

If the project later moves to **Vercel Pro**, you can restore native crons by
creating `vercel.json` with:

```json
{ "crons": [
  { "path": "/api/bot/cron/drain",   "schedule": "* * * * *" },
  { "path": "/api/tiktok/cron/poll", "schedule": "*/2 * * * *" }
] }
```

> Until a scheduler is wired up, the Facebook bot still answers comments in real time
> (the webhook is push-based) — only *throttle-deferred* comments wait. The TikTok bot,
> however, **does not work at all without a scheduler**, since polling is its only input.
